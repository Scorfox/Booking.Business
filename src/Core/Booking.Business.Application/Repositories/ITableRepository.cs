...
public interface ITableRepository : IBaseRepository<Table>
{
    ...
    public Task<List<Table>> GetTablesList(int offset, int limit, CancellationToken token = default);
    public Task<int> GetTablesTotalCount(CancellationToken token = default);
